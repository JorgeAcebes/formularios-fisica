...
&\text{Nº dets. Slater = nº comb. posibles (deg.)\;\; Entre \{\}$^{(\iota)}$ desdobles spin, desarrolla comb.}\\
...
\begin{array}{ccc}
     \text{Asemeja funs. esp}  \\
      \text{a $\psi_{\pm}$ y sustituye}\\[5pt]
      E=\sum_i E_{n_i}
\end{array}
...
\end{array}
\right.   \\[-5pt]
&E_{\text{config.}}= \Sigma_i E_{n_i} \;\;\; E_{\text{fund.}}=E_{\text{config. fund.}}\;\;\;\text{Config. fund.: aquella que minimice $E$}\\
...
&\text{DEG (entre [ ]):\; térm:}(2L+1)(2S+1)\;\;\;\text{nivel:}(2J+1)\;\;\;\text{\textit{h}$f$:}(2J+1)(2I+1)\;\;\;\text{F:}(2F+1) \\
...
&\scalebox{0.8}{\text{1º $M_S$ 2º $M_L$ (fina según semillenado). Write posibles $m_l m_s$ en capa $l$, elige $\chi$ ests. cumpliendo rules y Pauli.}}
\end{align*} 
\vspace{-10pt}
\begin{align*}
&\text{HUND: 1. Térms. $\uparrow$S(multip)\;$\downarrow$E \;\;2. S fijo, $\uparrow$L\,\;$\downarrow$E \;\;\; 3.Subcapa $\displaystyle \scalebox{0.7}{$\stackrel{>}{<}$}$\ 
...
\\
\vspace{-7pt}
&\scalebox{0.8}{\text{Si llenado semilleno: $\nexists$ desdoblamiento, todos niveles misma energía p.q. $\tilde{A}$=0 si subcapa semillena ($E\neq E(j)$)}}
\end{align*}
\begin{align*}
...
\end{array}
\right.\\
&\text{Indica distinta $\tilde{A}$ en función de $n, L,S$. Valora $\tilde{A}(\gamma,L,S)$ positiva o negativa según Hund.}\\
...
&\text{para varios e$^-$: }j_1\!=\!j_2\!\neq\!j_3: \text{ degs. iguales  y distinto por separado, then multiply} \\
...
&\text{Términos espectrales: } (j_1,j_2,..)  \xrightarrow{H_{\text{fina}}}\text{niveles espectrales: }(j_1,j_2,..)_J\\
